...
data <- read.csv("data/raw-data/araneae.csv", h = T, sep = ";", fileEncoding="latin1")
...
mymat <- data %>%
  ...
  complete(STATION, REPETITION := 1:6, fill = list(presence = 0)) %>%
...
# Boucle pour chaque station
  for (station in unique(mymat$STATION)) {
    ...
    site_data <- mymat %>%
      ...
      select(-c(STATION, METHODE, REPETITION, HABITAT))
    ...
    accumulation_data[[paste(station, method, sep = "_")]] <- data.frame(
      methode = method,
      station = station,
